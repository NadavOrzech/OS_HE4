...
void check_and_split(meta_data* current, size_t size){
    ...
    if((int)(current->block_size)<((int)size+ALIGNED_META_DATA+LARGE_ENOUGH))
        return;

    meta_data* new_meta_data=(meta_data*)current->start_of_alloc;             //inserts new meta_data to list
    (char*)new_meta_data += size;
    void* retval = std::memcpy(new_meta_data,current, ALIGNED_META_DATA);
    if(retval==NULL)
        return;

    new_meta_data->next_ptr=NULL;
    new_meta_data->prev_ptr=NULL;
    printf("Check and Split, block is %d\n", (int)(current->block_size));
