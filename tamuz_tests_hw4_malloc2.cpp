...
void test_malloc_then_free() {
	...
	const byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
			{{1,2,3}, {11,12,13,14,15,16,17,18}, {20}, {1,2,3,4,5,6,7}, {LAST}};
	...
	p[0] = static_cast<byte*>(malloc(BLOCK_SIZES[0]));
	add_expected_block(expected, BLOCK_SIZES[0]);
//	printf("Num_alocated_blocks: %d", _num_allocated_blocks());
//    printf("initial.allocated_blocks: %d", initial.allocated_blocks);
//    printf("expected.allocated_blocks: %d", expected.allocated_blocks);
	...
	/* Allocate block 1 */
	p[1] = static_cast<byte*>(malloc(BLOCK_SIZES[1]));
	...
	/* Allocate block 2 */
	p[2] = static_cast<byte*>(malloc(BLOCK_SIZES[2]));
	...
	/* Allocate block 3 */
	p[3] = static_cast<byte*>(malloc(BLOCK_SIZES[3]));
...
void test_reuse_after_free() {
	...
	byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
			{{1,2,3}, {11,12,13,14,15,16,17,18}, {20}, {21,22,23,24,25,26,27},
			 {31,32,33,34,35,36,37,38,39}, {91, 92}, {LAST}};
	...
	for (int i=0; i<4; ++i) {
		p[i] = static_cast<byte*>(malloc(BLOCK_SIZES[i]));
	...
	p[1] = static_cast<byte*>(malloc(sizeof(byte)*5));
	...
	p[3] = static_cast<byte*>(malloc(sizeof(byte)*7));
	...
	p[4] = static_cast<byte*>(malloc(BLOCK_SIZES[4]));
	...
	/* reuse block 0: */
	p[0] = static_cast<byte*>(malloc(sizeof(byte)));
	...
	*(p[0]) = 200;
	...
	/* allocate last block: */
	p[5] = static_cast<byte*>(malloc(BLOCK_SIZES[5]));
...
void test_calloc() {
	...
	byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
			{{10}, {20,21}, {30,31,32}, {LAST}};
	const byte ZEROS[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] = {{},{},{},{LAST}};
	byte *heap = static_cast<byte*>(sbrk(0));
	HeapState initial;
	get_initial_state(initial);
...
void test_realloc() {
	...
	byte DATA[BLOCK_MAX_COUNT][BLOCK_MAX_SIZE] =
			{{1,2,3}, {1,2,3,4}, {11,12,13,14,15}, {LAST}};
	byte *heap = static_cast<byte*>(sbrk(0));
	HeapState initial;
	get_initial_state(initial);
...
void test_failures() {
	...
	assert(!malloc(0));
	assert(!malloc(long(1e8 + 10)));
	assert(!calloc(sizeof(byte), 0));
	assert(!calloc(sizeof(byte), (long(1e8 + 10))));
	assert_state(initial, expected);
